...
def send_user_profile(chat_id, user_id):
    ...
    # Если имя найдено в базе данных, используем его
    if result:
        username = result[0]
    else:
        # Если имя не найдено, используем user_id
...
def process_feedback(chat_id, user_id):
    ...
    inline_keyboard = InlineKeyboardMarkup(row_butt)
...
def save_feedback(chat_id, user_id, num):
...
def cancel_order(chat_id, user_id):
    ...
    btn_profile = InlineKeyboardButton("Личный кабинет", callback_data="profile")
    inline_keyboard.add(btn_restaurant, btn_profile)
    bot.send_message(chat_id, "Ваш заказ отменен.")
